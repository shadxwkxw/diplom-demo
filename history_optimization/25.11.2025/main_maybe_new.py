...
def run_simulation():
    ...
    if tls_id not in tls_ids:
        ...
        if not tls_id:
            ...
            # === ОТКЛЮЧАЕМ ВСТРОЕННУЮ АДАПТИВНУЮ ЛОГИКУ SUMO (только в режиме opt) ===
    if enable_optimization:
        print(f"Режим opt: переводим светофор {tls_id} в полностью статический режим (type=static)")
        try:
            # Получаем текущую программу
            all_logics = traci.trafficlight.getAllProgramLogics(tls_id)
            current_logic = all_logics[0]  # обычно первая — активная

            # Создаём новую статическую программу с фиксированными длительностями
            static_phases = []
            for phase in current_logic.phases:
                # Берём текущую длительность, но не меньше 25 сек (чтобы не было микро-фаз)
                duration = max(phase.duration, 25)
                static_phase = traci.trafficlight.Phase(
                    duration=duration,
                    state=phase.state,
                    minDur=duration,   # ← КЛЮЧЕВОЕ: minDur = maxDur → actuated выключается полностью
                    maxDur=duration
                )
                static_phases.append(static_phase)

            # Создаём новую статическую логику
            static_logic = traci.trafficlight.Logic(
                programID="fixed_baseline_for_opt",
                type=0,  # 0 = static (полностью фиксированный цикл)
                currentPhaseIndex=traci.trafficlight.getPhase(tls_id),  # сохраняем текущую фазу
                phases=static_phases
            )

            # Применяем
            traci.trafficlight.setCompleteRedYellowGreenDefinition(tls_id, static_logic)
            traci.trafficlight.setProgram(tls_id, "fixed_baseline_for_opt")

            print(f"Успешно переведено в static. Начальные длительности фаз: {[p.duration for p in static_phases]}")
            print("Теперь наш optimize_phases() будет реально улучшать ситуацию!")

        except Exception as e:
            print(f"Не удалось перевести светофор в static режим: {e}")
            print("Продолжаем с текущей логикой — opt будет хуже baseline, но не упадёт")
